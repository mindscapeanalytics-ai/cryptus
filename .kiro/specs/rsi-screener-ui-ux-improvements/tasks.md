...
- [x] 4.1 Create `QuietHoursSection` component in `components/quiet-hours-section.tsx`
  - [x] 4.1.1 Add enable/disable toggle
  - [x] 4.1.2 Create time pickers for start/end hours (0-23)
  - [x] 4.1.3 Build visual 24-hour timeline with shaded active region
  - [x] 4.1.4 Add explanation text about high/critical priority bypass
- [x] 4.2 Integrate with `coinConfig` persistence
  - [x] 4.2.1 Save to `quietHoursEnabled`, `quietHoursStart`, `quietHoursEnd`
  - [x] 4.2.2 Load existing settings on modal open
- [x] 4.3 Create `QuietBadge` component
  - [x] 4.3.1 Display "Quiet" badge on symbols with active quiet hours
  - [x] 4.3.2 Add to `SymbolCell` component
  - [x] 4.3.3 Check current time against quiet hours range
- [x] 4.4 Add to symbol settings modal
  - [x] 4.4.1 Create new section in `CoinConfigModal`
  - [x] 4.4.2 Ensure mobile-friendly layout
...
- [x] 5.1 Create `AlertHistoryPanel` component in `components/alert-history-panel.tsx`
  - [x] 5.1.1 Display alerts in reverse chronological order
  - [x] 5.1.2 Show symbol, timeframe, type, value, price, timestamp
  - [x] 5.1.3 Add win/loss outcome badges (5m, 15m, 1h)
  - [x] 5.1.4 Display priority with color coding
- [x] 5.2 Create `AlertFilters` component
  - [x] 5.2.1 Add filter dropdowns for alert type
  - [x] 5.2.2 Add date range picker
  - [x] 5.2.3 Implement filter logic
- [x] 5.3 Create `AlertDetailModal` component
  - [x] 5.3.1 Show full alert context
  - [x] 5.3.2 Display conditional alert conditions if applicable
  - [x] 5.3.3 Show signal narration if available
- [x] 5.4 Match alerts with signal snapshots
  - [x] 5.4.1 Cross-reference alert timestamps with signal tracker data
  - [x] 5.4.2 Display outcome status
- [x] 5.5 Enhance existing alert panel in `ScreenerDashboard`
  - [x] 5.5.1 Replace simple list with enhanced panel
  - [x] 5.5.2 Add filter controls
  - [x] 5.5.3 Ensure mobile responsiveness
...
- [x] 6.1 Create `PrioritySelector` component in `components/priority-selector.tsx`
  - [x] 6.1.1 Add dropdown with options: low, medium, high, critical
  - [x] 6.1.2 Display behavior explanation for each level
  - [x] 6.1.3 Show current selection
- [x] 6.2 Create `SoundSelector` component in `components/sound-selector.tsx`
  - [x] 6.2.1 Add dropdown with options: default, soft, urgent, bell, ping
  - [x] 6.2.2 Add "Preview Sound" button
  - [x] 6.2.3 Implement sound playback using existing audio context
- [x] 6.3 Create `PriorityBadge` component
  - [x] 6.3.1 Display color-coded badge for non-default priorities
  - [x] 6.3.2 Add to `SymbolCell` component
- [x] 6.4 Integrate with `coinConfig` persistence
  - [x] 6.4.1 Save to `priority` and `sound` fields
  - [x] 6.4.2 Load existing settings
- [x] 6.5 Add to symbol settings modal
  - [x] 6.5.1 Create new section for alert customization
  - [x] 6.5.2 Ensure proper layout and spacing
...
- [x] 7.1 Create `FundingRateCell` component in `components/funding-rate-cell.tsx`
  - [x] 7.1.1 Display funding rate with color coding (green positive, red negative)
  - [x] 7.1.2 Add tooltip with annualized rate
  - [x] 7.1.3 Handle missing data with "-"
- [x] 7.2 Create `OrderFlowIndicator` component in `components/order-flow-indicator.tsx`
  - [x] 7.2.1 Display bullish/bearish/neutral indicator
  - [x] 7.2.2 Add tooltip with buy/sell volume breakdown
  - [x] 7.2.3 Use existing `useDerivativesIntel()` hook
- [x] 7.3 Create `LiquidationBadge` component in `components/liquidation-badge.tsx`
  - [x] 7.3.1 Display badge for large liquidations
  - [x] 7.3.2 Show size and direction (long/short)
  - [x] 7.3.3 Add animation for new liquidations
- [x] 7.4 Add derivatives columns to screener table
  - [x] 7.4.1 Add to `ScreenerRow` component
  - [x] 7.4.2 Add to `OPTIONAL_COLUMNS` (default hidden)
  - [x] 7.4.3 Add column toggle in settings
- [x] 7.5 Integrate with existing `useDerivativesIntel()` hook
  - [x] 7.5.1 Pass derivatives data to row components
  - [x] 7.5.2 Handle real-time updates
  - [x] 7.5.3 Ensure performance with memoization
...
- [x] 8.1 Create `PushNotificationWizard` component in `components/push-notification-wizard.tsx`
  - [x] 8.1.1 Implement multi-step modal (3 steps)
  - [x] 8.1.2 Step 1: Benefits explanation with icons
  - [x] 8.1.3 Step 2: Browser permission prompt
  - [x] 8.1.4 Step 3: Success confirmation with test notification option
- [x] 8.2 Implement onboarding trigger logic
  - [x] 8.2.1 Check localStorage for `hasSeenPushOnboarding`
  - [x] 8.2.2 Show modal on first visit
  - [x] 8.2.3 Add "Skip" and "Next" buttons
- [x] 8.3 Integrate with existing `usePushNotifications()` hook
  - [x] 8.3.1 Call `subscribe()` on permission grant
  - [x] 8.3.2 Send test notification on completion
  - [x] 8.3.3 Handle denied state with instructions
- [x] 8.4 Add push notification controls to settings
  - [x] 8.4.1 Display current status (active/inactive/denied/unsupported)
  - [x] 8.4.2 Add "Send Test Notification" button
  - [x] 8.4.3 Add "Re-enable" instructions for denied state
- [x] 8.5 Test on multiple browsers and devices
