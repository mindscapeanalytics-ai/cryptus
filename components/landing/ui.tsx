...
import { ChevronRight, ArrowUpRight, Lock, Zap, MapPin, Mail, Phone } from 'lucide-react';
...
export function PersonaCard({ icon, role, needs, impact }: { icon: React.ReactNode; role: string; needs: string; impact: string }) {
  return (
    <motion.div
      whileHover={{ y: -5 }}
      whileTap={{ scale: 0.98 }}
      className="p-6 sm:p-10 rounded-[28px] sm:rounded-[32px] bg-white/[0.02] border border-white/10 hover:border-[#39FF14]/30 hover:bg-[#39FF14]/[0.02] transition-all flex flex-col"
    >
      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-white/5 flex items-center justify-center mb-6 sm:mb-8 text-slate-400 group-hover:text-[#39FF14]">
  ...
  return (
    <div className={cn(
      ...
      status === 'pro'
        ...
        : "bg-white/[0.01] border-white/5 opacity-60 grayscale hover:grayscale-0 hover:opacity-100 font-medium"
    )}>
      <div className="flex items-center gap-4 mb-8">
        <div className={cn(
        ...
        {points.map((p, i) => (
          <div key={i} className="flex items-start gap-3 group/point">
            <div className={cn(
              ...
              status === 'pro' ? "bg-[#39FF14] animate-pulse" : "bg-slate-700"
            )} />
...
export function FeatureCard({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (
    <motion.div
      whileHover={{ y: -8 }}
      className="p-6 sm:p-8 md:p-10 rounded-3xl sm:rounded-[40px] bg-white/[0.02] border border-white/5 hover:border-[#39FF14]/30 hover:bg-[#39FF14]/[0.02] transition-all group relative overflow-hidden h-full"
    >
      ...
      <h3 className="text-xl sm:text-2xl font-black text-white tracking-tight mb-3 sm:mb-5 relative z-10 uppercase">{title}</h3>
...
export function StatBox({ value, label, highlight = false }: { value: string; label: string; highlight?: boolean }) {
  return (
    <motion.div
      ...
      className={cn(
        "p-5 sm:p-7 md:p-10 rounded-3xl sm:rounded-[40px] border flex flex-col items-center justify-center text-center gap-1.5 sm:gap-2 h-full",
        highlight
...
export function PlanCard({ tier, price, features, highlight = false }: { tier: string; price: string; features: string[]; highlight?: boolean }) {
  return (
    <motion.div
      whileHover={{ y: -10 }}
      className={cn(
...
export function AnalysisItem({ title, desc }: { title: string; desc: string }) {
    return (
      <div className="group border-l-2 border-white/10 pl-5 sm:pl-8 py-1 sm:py-3 hover:border-[#39FF14] transition-colors relative">
        ...
        <p className="text-slate-500 text-[11px] sm:text-[13px] font-bold leading-relaxed relative z-10 group-hover:text-slate-400 transition-colors">{desc}</p>
      </div>
    );
}
...
export function LeadServiceCard({
  ...
  return (
    <motion.div
      whileHover={{ y: -6 }}
      className="rounded-[2.5rem] border border-white/10 bg-[#0a0f1a] p-8 sm:p-10 flex flex-col h-full group hover:border-[#39FF14]/30 hover:bg-[#39FF14]/[0.02] transition-all"
    >
      <div className="w-12 h-12 rounded-2xl border border-[#39FF14]/30 bg-[#39FF14]/10 text-[#39FF14] flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
        {icon}
      </div>
      <h3 className="text-xl font-black tracking-tight text-white mb-4">{title}</h3>
      <p className="text-sm text-slate-400 leading-relaxed flex-1 font-medium">{desc}</p>
      <a
        href={href}
        target="_blank"
        rel="noreferrer"
        className="mt-8 inline-flex items-center gap-2 text-[#39FF14] text-[10px] font-black uppercase tracking-[0.2em] border-b border-[#39FF14]/0 hover:border-[#39FF14] transition-all w-fit"
      >
