...
export async function GET(request: Request, { params }: { params: { symbol: string } }) {
  try {
    ...
    // We use the guest-accessible path (limited symbols)
    const result = await getScreenerData(1, { 
      smartMode: true, 
      prioritySymbols: [symbol],
      exchange: 'binance' 
    });

    const entry = result.data.find(e => e.symbol === symbol) || {
      price: 0,
      signal: 'neutral',
      strategyScore: 0,
    ...
    const isOversold = entry.signal === 'oversold' || entry.strategyLabel.toLowerCase().includes('buy');
    const isOverbought = entry.signal === 'overbought' || entry.strategyLabel.toLowerCase().includes('sell');
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
             ...
             <div style={{ 
               ...
               <span style={{ color: themeColor, fontSize: '32px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '4px' }}>
                 {entry.strategyLabel.toUpperCase()}
               </span>
             </div>
          </div>
          ...
          <div style={{ 
            ...
            paddingTop: '30px'
             ...
             <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <span style={{ color: 'white', fontSize: '32px', fontWeight: 700 }}>{entry.rsi15m?.toFixed(1) || '--.-'}</span>
             </div>
             <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                <span style={{ color: '#64748b', fontSize: '18px', fontWeight: 900, textTransform: 'uppercase' }}>Strategy Score</span>
                <span style={{ color: themeColor, fontSize: '32px', fontWeight: 700 }}>{entry.strategyScore}</span>
             </div>
          </div>
